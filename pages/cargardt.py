...
def verificar_permiso_pagina(modulo, submodulo=None):
    ...
    # Forzamos una lectura estricta a booleano real
    ...
    # Si falta el permiso del módulo O el del submódulo, bloqueamos de inmediato
    if not permiso_modulo or not permiso_sub:
            ...
            ">
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 6px;">
                    ...
                    <span style="color: #FFD700; font-size: 13px; font-weight: 900; letter-spacing: 1.5px; text-transform: uppercase;">
                        ACCESS RESTRICTED // {tipo_bloqueo}
                    </span>
                </div>
                <div style="font-size: 11px; color: rgba(255,255,255,0.7); font-weight: 600; padding-left: 20px;">
                    No cuentas con los permisos necesarios para acceder a: <b style="color: white; text-transform: uppercase;">{item_fallido}</b>.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        with col_regresar_m:
            if st.button("REGRESAR AL INICIO", key=f"btn_regresar_{modulo}_{submodulo or 'mod'}", use_container_width=True):
                st.switch_page("dashboard.py")
                
        # ESTE STOP ES SAGRADO: Frena la ejecución para que se quede viendo la tarjeta y no se vaya al dashboard
        st.stop()

    return True

# Blindaje del módulo y submódulo para esta página
