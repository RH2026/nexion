    ...
    with main_container:
        ...
        # 5. HUB LOG
            ...
            if st.session_state.menu_sub == "SMART ROUTING":
                ...
                # --- 3. ÁREA DE CARGA Y PREPARACIÓN DE DATOS ---
                uploaded_file = st.file_uploader(":material/upload_file: SUBIR ARCHIVO ERP (XLSX, CSV)", type=["xlsx", "csv"], label_visibility="collapsed")
                ...
                if uploaded_file is not None:
                    try:
                        ...
                        df_raw.columns = [str(c).strip().replace('\n', '').replace('\t', '') for c in df_raw.columns]
                        
                        # Identificación de columnas clave
                        col_folio = next((c for c in df_raw.columns if c.lower() == 'factura'), None)
                        col_transporte = next((c for c in df_raw.columns if c.lower() == 'transporte'), None)
                        col_direccion = next((c for c in df_raw.columns if 'direccion' in c.lower() or 'destino' in c.lower()), None)
            
                        # Si no hay coincidencia exacta de folio, buscamos la que contenga la palabra o usamos la primera
                        if not col_folio:
                            col_folio = next((c for c in df_raw.columns if 'factura' in c.lower()), df_raw.columns[0])
                        if not col_transporte:
                            col_transporte = next((c for c in df_raw.columns if 'transporte' in c.lower()), None)
            
                        st.toast(f"ERP DETECTADO -> Folio: {col_folio}", icon="🔍")
            
                        # --- PANEL DE CONTROL DE FILTRADO ---
                        st.markdown("<br>", unsafe_allow_html=True)
                        col_left, col_right = st.columns([1, 2], gap="large")
                        
                        with col_left:
                            st.markdown(f"<p class='op-query-text' style='text-align:left !important;'>FILTROS DE RANGO</p>", unsafe_allow_html=True)
                            serie_folios = pd.to_numeric(df_raw[col_folio], errors='coerce').dropna()
                            ...
                            # Aplicamos el filtro numérico
                            df_raw[col_folio] = pd.to_numeric(df_raw[col_folio], errors='coerce')
                            df_rango = df_raw[(df_raw[col_folio] >= inicio) & (df_raw[col_folio] <= final)].copy()
            
                        with col_right:
                            st.markdown(f"<p class='op-query-text' style='text-align:left !important;'>SELECCIÓN DE FOLIOS</p>", unsafe_allow_html=True)
                            if not df_rango.empty:
                                # Agrupamos para el selector
                                cols_to_show = [col_folio]
                                if col_transporte: cols_to_show.append(col_transporte)
                                
                                info_folios = df_rango.drop_duplicates(subset=[col_folio])[cols_to_show].copy()
                                ...
                                edited_df = st.data_editor(
                                    ...
                                    hide_index=True, height=250, use_container_width=True, key="editor_unificado"
                                )
                            else:
                                st.warning("Sin datos en el rango.")
                                edited_df = pd.DataFrame()
            
                        # --- 4. MOTOR SMART (EL CORAZÓN DE LA INTEGRACIÓN) ---
                        st.markdown("---")
                        if not edited_df.empty:
                            folios_finales = edited_df[edited_df["Incluir"] == True][col_folio].tolist()
                            
                            if st.button("🚀 EJECUTAR SMART ROUTING (ANALIZAR LOGÍSTICA)", use_container_width=True):
                                # Filtramos los datos finales para el análisis
                                df_final = df_rango[df_rango[col_folio].isin(folios_finales)].copy()
                                
                                if not df_final.empty:
                                    # Lógica de Recomendación
                                    def motor_prioridad(row):
                                        addr = str(row.get(col_direccion, "")).upper() if col_direccion else ""
                                        # Regla GDL Local
                                        if any(loc in addr for loc in ["GDL", "GUADALAJARA", "ZAPOPAN", "TLAQUEPAQUE", "TONALA"]):
                                            return "LOCAL"
                                        return d_flet.get(addr, "SIN HISTORIAL")
            
                                    df_final['RECOMENDACION'] = df_final.apply(motor_prioridad, axis=1)
                                    df_final['COSTO'] = df_final.apply(lambda r: 0.0 if r['RECOMENDACION'] == "LOCAL" else d_price.get(str(r.get(col_direccion, "")), 0.0), axis=1)
                                    df_final['FECHA_HORA'] = datetime.now().strftime("%Y-%m-%d %H:%M")
                                    
                                    # Reordenar columnas para poner lo importante al principio
                                    cols_sis = [col_folio, 'RECOMENDACION', 'COSTO', 'FECHA_HORA']
                                    otras = [c for c in df_final.columns if c not in cols_sis]
                                    st.session_state.df_analisis = df_final[cols_sis + otras]
                                    st.success("Análisis completado con éxito.")
            
                        # --- 5. RESULTADOS Y SELLADO ---
                        if "df_analisis" in st.session_state:
                            st.markdown("### :material/analytics: RESULTADOS Y EDICIÓN")
                            modo_edicion = st.toggle(":material/edit_note: HABILITAR EDICIÓN MANUAL")
                            ...
                            # --- ACCIONES DE DESCARGA Y SELLADO ---
                            st.markdown("<br>", unsafe_allow_html=True)
                            c1, c2, c3 = st.columns(3)
                            
                            with c1:
                                st.download_button(":material/download: EXCEL ANALIZADO", p_editado.to_csv(index=False).encode('utf-8-sig'), "Analisis_Nexion.csv", use_container_width=True)
                            with c2:
                                if st.button(":material/push_pin: FIJAR CAMBIOS", use_container_width=True):
                                    st.session_state.df_analisis = p_editado
                                    st.toast("Cambios guardados", icon="📌")
                            with c3:
                                if st.button(":material/save: GUARDAR EN HISTORIAL", use_container_width=True):
                                    st.snow()
                            
                            # PANEL DE SELLADO
                            st.markdown("---")
                            st.markdown("<h3 style='font-size: 16px; color: white;'>:material/print: SISTEMA DE SELLADO</h3>", unsafe_allow_html=True)
                            
                            with st.expander(":material/settings: CALIBRACIÓN DE SELLO"):
                                col_x, col_y = st.columns(2)
                                ajuste_x = col_x.slider("Horizontal (X)", 0, 612, 510)
                                ajuste_y = col_y.slider("Vertical (Y)", 0, 792, 760)
            
                            col_p, col_d = st.columns(2)
                            with col_p:
                                if st.button(":material/article: GENERAR SELLOS (PAPEL)", use_container_width=True):
                                    sellos = p_editado['RECOMENDACION'].tolist()
                                    pdf_out = generar_sellos_fisicos(sellos, ajuste_x, ajuste_y)
                                    st.download_button("Descargar PDF", pdf_out, "Sellos_Fisicos.pdf", use_container_width=True)
                            ...
                            with col_d:
                                ...
                                if pdfs and st.button("EJECUTAR ESTAMPADO DIGITAL"):
                                    mapa = pd.Series(p_editado.RECOMENDACION.values, index=p_editado[col_folio].astype(str)).to_dict()
                                    z_buf = io.BytesIO()
                                    ...
                                    st.download_button(":material/folder_zip: DESCARGAR ZIP", z_buf.getvalue(), "Facturas_Digitales.zip", use_container_width=True)
            
                    except Exception as e:
                        st.error(f"Error en procesamiento: {e}")
                        
                else:
                    # Estado de espera (Animación NEXION)
                    st.markdown(f"""
                        <div style="height: 300px; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                            <div style="width: 20px; height: 20px; background-color: #54AFE7; border-radius: 50%; box-shadow: 0 0 20px #54AFE7; animation: pulse 2s infinite;"></div>
                            <p style="color: #54AFE7; font-family: monospace; letter-spacing: 5px; font-size: 10px; margin-top: 30px;">ESPERANDO DATA ERP...</p>
                        </div>
