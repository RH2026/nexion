...
# ============================================================
st.set_page_config(
    page_title="JYPESA | Logistics - Envio de Muestras",
    layout="wide",
...
# ============================================================
render_layout(modulo_actual="REPORTES", submodulo_actual="ENVIO DE MUESTRAS")
...
def generar_etiquetas_limpias(reg_datos, total_etqs, factura_val, transporte_val):
    ...
    for i in range(total_etqs):
        ...
        if y_inicio_direccion > y_offset + 4.3*units.cm: y_inicio_direccion = y_offset + 4.3*units.cm
        if y_inicio_direccion < y_offset + 2.9*units.cm: y_inicio_direccion = y_offset + 2.9*units.cm
    ...
    if es_admin_general or usuario_logeado in ["Rigoberto", "JMoreno"]:
        ...
        with t1:
            ...
            if not df_actual.empty:
                ...
                st.divider()
                # ============================================================
                # ADMINISTRACIÓN DEL FOLIO - ANCHO COMPLETO
                # ============================================================
                st.markdown(
        ...
        with t2:
            if not df_actual.empty:
                ...
                with c3:
                    if st.button(":material/update: ACTUALIZAR", use_container_width=True, key="btn_actualizar_tab"): st.rerun()
            else:
                st.info("No hay registros todavía.")
