...
# --- DISEÑO DE IMPRESIÓN PROFESIONAL (El que te gustó) ---
...
with c1:
    paqueteria_sel = st.selectbox(":material/local_shipping: FLETERA / PAQUETERÍA", 
                                  ["ONE PAQUETERIA", "FEDEX", "ESTAFETA", "DHL", "PAQUETEXPRESS", "TRESGUERRAS"])
with c2:
    tipo_incidencia = st.selectbox(":material/report_problem: TIPO DE RECLAMO", 
                                   ["Faltante", "Extravío", "Siniestro / Daño Total", "Daño Parcial"])
with c3:
    f_fecha = st.date_input(":material/calendar_today: FECHA", date.today())

# SECCIÓN 2: DETALLES TÉCNICOS
c4, c5, c6 = st.columns([1, 1.5, 1])
with c4:
    f_cajas = st.number_input("CANTIDAD CAJAS", min_value=1, value=1)
with c5:
    f_codigos = st.text_input("CÓDIGOS AFECTADOS", placeholder="Ej: 4052-L20, 4052-L18")
with c6:
    f_guia = st.text_input("NÚMERO DE GUÍA", "JALGDL ")

f_monto = st.text_input("MONTO A RECLAMAR (Solo número)", placeholder="3,410")
...
# ÁREA DE EDICIÓN
st.subheader(":material/edit: CUERPO DE LA CARTA")
cuerpo_final = st.text_area("Edita el texto final si es necesario:", value=texto_defecto, height=220)

# BOTÓN DE IMPRESIÓN
if st.button(":material/print: GENERAR E IMPRIMIR RECLAMO", use_container_width=True, type="primary"):
    ...
    st.success(f"Documento de {tipo_incidencia} generado.")
