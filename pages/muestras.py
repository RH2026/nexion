...
def render_logistica_flow_mid(data):
    ...
    <head>
        ...
        <style>
            /* El fondo de tu app aplicado aquí */
            body {{ 
                ...
                <div class="glass-card rounded-2xl p-4 flex flex-col justify-between {"status-delivered" if item['estatus'] == "ENTREGADA" else "status-pending"}">
                    <div>
                        <div class="flex justify-between items-center mb-3">
                            <span class="text-[9px] font-bold text-white/70 bg-black/30 px-2 py-0.5 rounded uppercase tracking-wider">
                                {item['semana']}
                            </span>
                        ...
                        <div class="space-y-2.5 bg-black/40 rounded-xl p-3 border border-white/5 shadow-inner">
                            <div class="flex items-center justify-between">
                                <span class="text-[9px] text-white uppercase font-extrabold tracking-widest">Volumen</span>
                                <span class="text-sm font-bold text-white">{item['cantidad']}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-[9px] text-white uppercase font-extrabold tracking-widest">Cita</span>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex justify-between mt-1.5 px-0.5">
                            <span class="text-[8px] font-bold text-white/40 uppercase tracking-tighter italic">{item['estatus']}</span>
                            <span class="text-[8px] font-bold text-white/60">{"100%" if item['estatus'] == "ENTREGADA" else "30%"}</span>
                        </div>
                    </div>
                </div>
                ''' for item in data])}
