...
def generar_etiquetas_nexion(df_datos):
    output = io.BytesIO()
    # 3 x 2 pulgadas exactas para la Zebra (7.62 cm de ancho x 5.08 cm de alto)
    w_rec, h_rec = 7.62 * cm, 5.08 * cm
    c = canvas.Canvas(output, pagesize=(w_rec, h_rec))
    
    margen_h = 0.3 * cm  # Margen lateral más limpio para aprovechar el ancho
    ...
    for index, row in df_datos.iterrows():
        ...
        for i in range(iteraciones):
            # Opcional: quitamos el rectángulo punteado de corte para impresión final limpia en la Zebra, 
            # o lo dejamos muy sutil si prefieres guía visual. Lo dejamos limpio para impresión térmica directa:
            # c.setDash(1, 2)
            ...
            dibujar_texto_bloque_pro(c, info_contacto, x_centro, h_rec - 0.6 * cm, w_util, "Helvetica", 4.5, 0.2 * cm, max_lineas=1)
            ...
            # --- NOMBRE DEL CLIENTE ---
            # Posicionado de forma óptima para que no choque con la línea superior
            y_termino_nombre = dibujar_texto_bloque_pro(c, nombre_final, x_centro, h_rec - 1.25 * cm, w_util, "Helvetica-Bold", 13.0, 0.4 * cm, max_lineas=2)
            ...
            dibujar_texto_bs = dibujar_texto_bloque_pro(c, direccion_final, x_centro, y_inicio_direccion, w_util, "Helvetica-Bold", 8.0, 0.3 * cm, max_lineas=3)

            # --- SECCIÓN INFERIOR (PIE DE PÁGINA / METRICAS) ---
            c.setLineWidth(0.5)
            c.setStrokeColorRGB(0, 0, 0)
            y_linea_pie = 0.95 * cm
