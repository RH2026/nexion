...
# --- 1. MOTOR DE EXCEL (REPLICA EXACTA DEL PDF) ---
def generar_excel_identico(enc, productos, dictamen_val, admin_nc, gastos):
    output = io.BytesIO()
    ...
    for i in range(10):
        if i < len(productos):
            ...
            if p.get('FECHA') and not pd.isnull(p['FECHA']):
                try: f_v = p['FECHA'].strftime('%d/%m/%Y')
                except: f_v = str(p['FECHA'])
    ...
    # D. BLOQUE ANALISTA E INCOMING (REPLICA EXACTA PDF)
    ...
    ws.merge_range(row+2, 0, row+2, 11, f'Gasto MO: {gastos[0]} | Hrs: {gastos[1]} | Cantidad: {gastos[3]}', std_f)
...
# --- 5. BOTONES DE ACCIÓN (SOLUCIÓN DEFINITIVA) ---
st.divider()
...
# El Excel se genera SÓLO cuando se presiona el botón para evitar el TypeError
if col2.button(":material/file_download: GENERAR Y DESCARGAR EXCEL", use_container_width=True):
    ex_data = generar_excel_identico(
    ...
    st.download_button(
        label="¡EXCEL LISTO! CLIC AQUÍ PARA GUARDAR", 
        data=ex_data, 
        file_name=f"CALIDAD_{f_nom}.xlsx", 
