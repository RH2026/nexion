    ...
    for df_source, nombre_f in [(df_t1, "TRES GUERRAS"), (df_t2, "TINY PACK"), (df_t3, "ONE")]:
        if df_source is not None:
            ...
            if not res.empty:
                ...
                # Estatus: Si no hay dato, información en proceso
                estatus = f.get("ESTATUS") or f.get("ESTATUS ENTREGA") or "INFORMACIÓN EN PROCESO"
                
                # Bultos: Agregamos Paquetes_Ampara para T3
                bultos = f.get("BULTOS") or f.get("PIEZAS") or f.get("Paquetes_Ampara") or "0"
                
                # Importe: Prioridad a Sub total _ Guia para T3
                importe = f.get("Sub total _ Guia") or f.get("TOTAL") or f.get("SUBTOTAL") or f.get("TOTAL_CARGO") or "0.00"
