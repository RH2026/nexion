...
st.markdown("""
    ...
    div[data-testid="stMetricValue"] { color: #FFFFFF; font-weight: 900; font-size: 2rem; }
...
try:
    ...
    st.title("📦 NEXION LOGISTICS | JYPESA PRO")
    
    c_f1, c_f2, c_f3 = st.columns([1, 1, 2])
    with c_f1:
        mes_sel = st.selectbox("📅 FILTRAR MES:", ["TODOS"] + sorted(df_gastos['MES'].unique().tolist()))
    with c_f2:
        flet_sel = st.selectbox("🚛 FLETERA:", ["TODAS"] + sorted(df_gastos['FLETERA'].unique().tolist()))
    with c_f3:
        search = st.text_input("🔍 BÚSQUEDA RÁPIDA:", "")
    ...
    # 5. RENDERIZADO DE KPIs CON TARGET 7.5%
    st.markdown("---")
    ...
    # 6. TABLA CON ESTILO CORPORATIVO
    st.markdown("---")
    st.subheader("📊 MATRIZ OPERATIVA DE ALTO RENDIMIENTO")
