...
def crear_lock():
    ...
    try:
        ...
        lock_data = {"usuario": current_user, "timestamp": ahora.strftime("%Y-%m-%d %H:%M:%S"), "hora": ahora.strftime("%H:%M:%S")}
        contenido = json.dumps(lock_data, indent=4)

        try:
            archivo = repo.get_contents(LOCK_FILE_PATH, ref="main")

            repo.update_file(path=LOCK_FILE_PATH, message=f"LOCK // {current_user}", content=contenido, sha=archivo.sha, branch="main")

        except:
            repo.create_file(path=LOCK_FILE_PATH, message=f"LOCK // {current_user}", content=contenido, branch="main")
...
def liberar_lock():
    ...
    try:
        ...
        repo.delete_file(path=LOCK_FILE_PATH, message=f"UNLOCK // {current_user}", sha=archivo.sha, branch="main")
...
bloqueado_por_otro = bool(lock_info and lock_info.get("usuario", "").upper() != current_user.upper())

puede_editar_efectivo = puede_editar and not bloqueado_por_otro and bool(TOKEN)
...
if bloqueado_por_otro:
    st.warning(f"⚠️ MÓDULO PAUSADO: sesión activa de **{lock_info.get('usuario','OTRO USUARIO')}**.")
...
def get_wallet_data_from_git():
    ...
    ejemplos = [
        {"Fecha": (ahora - timedelta(days=10)).strftime("%Y-%m-%d %H:%M"), "Tipo": "Ingreso", "Categoria": "Nómina", "Concepto": "Pago Quincena 1 JYPESA", "Monto": 35000.0, "Cuenta": "Caja Jypesa"},
        {"Fecha": (ahora - timedelta(days=8)).strftime("%Y-%m-%d %H:%M"), "Tipo": "Gasto", "Categoria": "Renta", "Concepto": "Renta Oficinas", "Monto": -18000.0, "Cuenta": "Santander"},
        {"Fecha": (ahora - timedelta(days=5)).strftime("%Y-%m-%d %H:%M"), "Tipo": "Ingreso", "Categoria": "Freelance / Proyectos", "Concepto": "Proyecto Xenocode UI", "Monto": 15000.0, "Cuenta": "Scottiabank"},
        {"Fecha": (ahora - timedelta(days=1)).strftime("%Y-%m-%d %H:%M"), "Tipo": "Gasto", "Categoria": "Supermercado", "Concepto": "Compras Semanales", "Monto": -3500.0, "Cuenta": "Cartera"}
    ]
    ...
    if TOKEN:
        try:
            ...
            try:
                ...
                df = pd.read_csv(io.StringIO(archivo.decoded_content.decode("utf-8")), keep_default_na=False)

            except:
                repo.create_file(path=FILE_PATH, message="INITIALIZE WALLET MATRIX", content=df.to_csv(index=False), branch="main")
...
def get_plan_data_from_git(force_reload=False):
    ...
    columnas = ["Fecha_Corte", "Semana", "Ingreso_Semanal", "Gastos_Fijos", "Ahorro_Semanal", "Disponible", "Usuario"]
    ...
    if TOKEN:
        try:
            ...
            try:
                ...
                repo.create_file(path=PLAN_FILE_PATH, message="INITIALIZE WEEKLY FINANCIAL PLAN", content=df.to_csv(index=False), branch="main")
...
if not df_actual.empty:
    df_actual["Mes"] = pd.to_datetime(df_actual["Fecha"], errors="coerce").dt.strftime("%Y-%m")
    mes_actual = datetime.now(tz_gdl).strftime("%Y-%m")
    df_month = df_actual[df_actual["Mes"] == mes_actual].copy()
else:
...
if not df_actual.empty:
    for cuenta in CUENTAS_MATRIX:
        saldos_actuales[cuenta] += pd.to_numeric(df_actual.loc[df_actual["Cuenta"] == cuenta, "Monto"], errors="coerce").fillna(0).sum()

total_general = sum(saldos_actuales.values())

inc_month = df_month.loc[df_month["Tipo"] == "Ingreso", "Monto"].sum() if not df_month.empty else 0

exp_month = abs(df_month.loc[df_month["Tipo"] == "Gasto", "Monto"].sum()) if not df_month.empty else 0
...
tab_kpi, tab_flujos, tab_registro, tab_plan = st.tabs(["KPI'S WALLET", "FLUJOS DE EFECTIVO", "REGISTRO NUBE", "PLAN SEMANAL"])
...
with tab_kpi:
    ...
    tarjetas_kpi = [
        (k1, "PATRIMONIO NETO", total_general, "#00E5FF", "BALANCE GLOBAL"),
        (k2, "INGRESOS MTD", inc_month, "#00FFAA", "FLUJO DE ENTRADA"),
        (k3, "EGRESOS MTD", exp_month, "#FF4B4B", "GASTOS DEL MES")
    ]

    for col, titulo, valor, color, sub in tarjetas_kpi:
        with col:
            st.markdown(f"""<div class="kpi-card"><div class="kpi-label">{titulo}</div><div class="kpi-value">${valor:,.2f}</div><div class="kpi-trend" style="color:{color}">{sub}</div><div class="neon-bar" style="background:linear-gradient(90deg,{color},transparent);"></div></div>""", unsafe_allow_html=True)

    st.markdown("<hr style='border-color:#34495E;'>", unsafe_allow_html=True)

    st.markdown("<p class='kpi-label'><span style='color:#00E5FF'>🔍</span> DISTRIBUCIÓN DE CAPITAL POR CUENTA</p>", unsafe_allow_html=True)

    nombres = list(saldos_actuales.keys())
    valores = list(saldos_actuales.values())
    colores = [CUENTAS_MATRIX[x]["color"] for x in nombres]

    fig = go.Figure(go.Bar(x=valores, y=nombres, orientation="h", marker=dict(color=colores, line=dict(color="#1D2A35", width=2)), text=valores, texttemplate="%{text:$,.2f}", textposition="auto", textfont=dict(color="#FFFFFF", size=12), hovertemplate="<b>%{y}</b><br>Saldo: %{x:$,.2f}<extra></extra>"))

    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, margin=dict(t=10, b=10, l=10, r=10), height=350, xaxis=dict(showgrid=True, gridcolor="#34495E", color="#8B9BB4", tickformat="$,.0f"), yaxis=dict(color="#E0E6ED", autorange="reversed"))
...
with tab_flujos:
    ...
    flujo1, flujo2 = st.columns([2, 1.5], gap="small")

    with flujo1:

        st.markdown("<p class='kpi-label'>TENDENCIA DE FLUJO <span style='color:#8B9BB4'>(MES ACTUAL)</span></p>", unsafe_allow_html=True)
        ...
        if not df_month.empty:
            ...
            fig_flow.add_trace(go.Scatter(x=df_daily.index, y=df_daily["Ingreso"], name="Ingresos", mode="lines", line=dict(width=3, color="#00FFAA"), fill="tozeroy", fillcolor="rgba(0,255,170,.05)"))

            fig_flow.add_trace(go.Scatter(x=df_daily.index, y=df_daily["Gasto"], name="Egresos", mode="lines", line=dict(width=3, color="#FF4B4B"), fill="tozeroy", fillcolor="rgba(255,75,75,.05)"))

            fig_flow.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", xaxis=dict(showgrid=False, color="#8B9BB4"), yaxis=dict(showgrid=True, gridcolor="#34495E", color="#8B9BB4"), legend=dict(orientation="h", y=1.1, x=.5, xanchor="center", font=dict(color="#E0E6ED")), margin=dict(t=10, b=10, l=10, r=10), height=350, hovermode="x unified")
    ...
    with flujo2:
        ...
        st.markdown("<p class='kpi-label'>ANÁLISIS DE CONSUMO POR CATEGORÍA</p>", unsafe_allow_html=True)

        if not df_month.empty:

            gastos = df_month[df_month["Tipo"] == "Gasto"].groupby("Categoria")["Monto"].sum().abs().reset_index().sort_values("Monto")

            if not gastos.empty:

                fig_cat = px.bar(gastos, x="Monto", y="Categoria", orientation="h", text_auto=",.0f")

                n = len(gastos)

                colors = ["#44B3E1" if i == n - 1 else "#4D93D9" if i >= n - 3 else "#215C98" for i in range(n)]

                fig_cat.update_traces(marker_color=colors, textposition="outside", hovertemplate="%{y}: $%{x:,.2f}")

                fig_cat.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", xaxis=dict(title=None, showgrid=True, gridcolor="#34495E", color="#8B9BB4", tickformat="$,.0f"), yaxis=dict(title=None, color="#E0E6ED"), margin=dict(t=10, b=10, l=10, r=10), height=350)
...
with tab_registro:
    ...
    if not TOKEN:
        st.warning("GITHUB_TOKEN no está configurado. El módulo funciona en modo lectura.")

    f1, f2, f3, f4 = st.columns([1, 1, 1, 1], gap="small")
    ...
    bloquear = not puede_editar_efectivo or bloqueado_por_otro or not TOKEN

    with b1:
        ingreso_sub = st.button("REGISTRAR INGRESO", icon=":material/save:", use_container_width=True, key="btn_ingreso_action", disabled=bloquear)

    with b2:
        gasto_sub = st.button("REGISTRAR GASTO", icon=":material/remove:", use_container_width=True, key="btn_gasto_action", disabled=bloquear)
    ...
    if not bloquear and (ingreso_sub or gasto_sub):
        ...
        if f_monto <= 0:
            ...
            try:
                ...
                if not lock_creado:
                    st.error("No fue posible tomar el control de escritura en GitHub.")
                    st.stop()
                ...
                df_latest = pd.read_csv(io.StringIO(archivo.decoded_content.decode("utf-8")), keep_default_na=False)
    ...
    st.markdown("<div style='color:#00FFAA;font-size:11px;font-weight:800;letter-spacing:1.5px;margin:25px 0 10px 0;'>REGISTROS DE CARTERA</div>", unsafe_allow_html=True)

    actualizar = st.button("ACTUALIZAR REGISTROS", use_container_width=True, key="btn_refresh_wallet")
    ...
    if df_registros.empty:
        ...
        if "Monto" in tabla.columns:
            tabla["Monto"] = pd.to_numeric(tabla["Monto"], errors="coerce")

        columnas = ["Fecha", "Tipo", "Categoria", "Concepto", "Monto", "Cuenta"]
...
with tab_plan:
    ...
    plan_cards = [
        (c1, "INGRESO SEMANAL", INGRESO_SEMANAL, "Ingreso disponible de la semana"),
        (c2, "GASTOS + AHORRO", TOTAL_APARTADO_SEMANAL, "Comprometido semanalmente"),
        (c3, "APARTADO MENSUAL", TOTAL_AHORRO_MENSUAL_SEM, "Ahorro proporcional semanal"),
        (c4, "DISPONIBLE", DISPONIBLE_SEMANAL, "Libre después de apartados")
    ]
    ...
    for col, titulo, valor, sub in plan_cards:
        ...
        with col:
            ...
            st.markdown(f"""<div class="plan-card"><div class="plan-title">{titulo}</div><div class="plan-value">${valor:,.2f}</div><div class="plan-sub">{sub}</div></div>""", unsafe_allow_html=True)
    ...
    st.markdown("<div style='color:#00FFAA;font-size:11px;font-weight:800;letter-spacing:1.5px;margin:18px 0 10px 0;'>CALENDARIO DE PAGOS MENSUALES</div>", unsafe_allow_html=True)

    pagos = [
        ("ABONO TV", 900.00, "25 SEP", "VIERNES", "Fondo asegurado con cobro del día"),
        ("INTERNET", 1100.00, "27 SEP", "DOMINGO", "Asegurar desde el viernes 25"),
        ("PRÉSTAMO", 1600.00, "28 SEP", "LUNES", "Asegurar desde el viernes 25")
    ]

    p1, p2, p3 = st.columns([1, 1, 1], gap="small")

    for col, pago in zip([p1, p2, p3], pagos):

        with col:

            concepto, monto, fecha, dia, estado = pago

            st.markdown(f"""<div class="plan-card" style="min-height:145px;"><div style="color:#00E5FF;font-size:10px;font-weight:800;letter-spacing:1.5px;">{concepto}</div><div style="color:#FFFFFF;font-size:25px;font-weight:700;margin-top:7px;">${monto:,.2f}</div><div style="color:#FFD700;font-size:11px;font-weight:700;margin-top:4px;">{fecha} · {dia}</div><div style="color:#8B9BB4;font-size:11px;margin-top:8px;">{estado}</div></div>""", unsafe_allow_html=True)
    ...
    st.markdown("<div style='color:#00FFAA;font-size:11px;font-weight:800;letter-spacing:1.5px;margin:22px 0 10px 0;'>DISTRIBUCIÓN DEL VIERNES DE PAGO</div>", unsafe_allow_html=True)
    ...
    df_distribucion = pd.DataFrame(
        ...
        columns=["CONCEPTO", "MONTO SEMANAL", "TIPO"]
    )

    st.dataframe(
        df_distribucion.style.format({"MONTO SEMANAL": "${:,.2f}"}),
        use_container_width=True,
        hide_index=True,
        height=330
    )
    ...
    resumen = [
        (r1, "GASTOS FIJOS", TOTAL_GASTOS_SEMANALES, "Casa + gasolina + consulta + hija"),
        (r2, "AHORRO SEMANAL", TOTAL_AHORRO_MENSUAL_SEM, "TV + préstamo + internet"),
        (r3, "LIBRE", DISPONIBLE_SEMANAL, "Disponible después de apartados")
    ]
    ...
    btn_corte, info_corte = st.columns([1, 2], gap="small")
    ...
    with btn_corte:
        ...
        registrar_corte = st.button(
            ...
            disabled=not TOKEN or not puede_editar_efectivo or bloqueado_por_otro
        )
    ...
    with info_corte:
        ...
        if bloqueado_por_otro:
            ...
            st.warning("Edición temporalmente bloqueada.")

        elif not TOKEN:

            st.warning("GITHUB_TOKEN no está configurado.")

        else:

            st.caption("El corte se almacenará en plan_financiero_semanal.csv.")
    ...
    if registrar_corte:
        ...
        try:
            ...
            if not lock_creado:
                ...
                st.error("No fue posible tomar el control de escritura.")
                st.stop()

            repo = Github(TOKEN).get_repo(REPO_NAME)

            fecha_corte = datetime.now(tz_gdl).strftime("%Y-%m-%d")
            ...
            if (
                ...
                st.warning(f"Ya existe un corte registrado para {fecha_corte}.")
                st.stop()

            ahora = datetime.now(tz_gdl)

            semana = f"{ahora.year}-W{ahora.isocalendar().week:02d}"
            ...
            df_plan = pd.concat([df_plan, nuevo], ignore_index=True)
            ...
            st.success(f"Corte {semana} registrado correctamente.")
            ...
            st.error(f"No fue posible guardar el corte: {e}")
    ...
    st.markdown("<div style='color:#00FFAA;font-size:11px;font-weight:800;letter-spacing:1.5px;margin:25px 0 10px 0;'>HISTORIAL DE CORTES SEMANALES</div>", unsafe_allow_html=True)

    df_hist = get_plan_data_from_git()

    if df_hist.empty:

        st.info("Todavía no existen cortes registrados.")
        ...
        if "Fecha_Corte" in df_hist.columns:
            df_hist = df_hist.sort_values("Fecha_Corte", ascending=False)
        ...
        columnas = [c for c in columnas if c in df_hist.columns]
