...
# NUEVO ARCHIVO PARA LA 4ª PESTAÑA
...
with tab_plan:
    ...
    with c1:
        st.markdown(
            f"""
            <div class="plan-card">
                <div class="plan-title">INGRESO SEMANAL</div>
                <div class="plan-value">${INGRESO_SEMANAL:,.2f}</div>
                <div class="plan-sub">Ingreso disponible de la semana</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    with c2:
        st.markdown(
            f"""
            <div class="plan-card">
                <div class="plan-title">GASTOS + AHORRO</div>
                <div class="plan-value">${TOTAL_APARTADO_SEMANAL:,.2f}</div>
                <div class="plan-sub">Comprometido semanalmente</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    with c3:
        st.markdown(
            f"""
            <div class="plan-card">
                <div class="plan-title">APARTADO MENSUAL</div>
                <div class="plan-value">${TOTAL_AHORRO_MENSUAL_SEM:,.2f}</div>
                <div class="plan-sub">Ahorro proporcional semanal</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    with c4:
        st.markdown(
            f"""
            <div class="plan-card">
                <div class="plan-title">DISPONIBLE</div>
                <div class="plan-value">${DISPONIBLE_SEMANAL:,.2f}</div>
                <div class="plan-sub">Libre después de apartados</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    for columna, pago in zip([p1, p2, p3], pagos_mensuales):

        with columna:

            st.markdown(
                f"""
                <div class="plan-card" style="min-height:145px;">
    ...
    df_distribucion = pd.DataFrame(
        [
            ["Gastos de Casa / Despensa", 1500.00, "Gasto Fijo"],
            ["Gasolina", 600.00, "Gasto Fijo"],
            ["Consulta", 350.00, "Gasto Fijo"],
            ["Gastos de tu Hija", 250.00, "Gasto Fijo"],
            ["Ahorro Abono TV", 225.00, "Ahorro Mensual"],
            ["Ahorro Abono Préstamo", 400.00, "Ahorro Mensual"],
            ["Ahorro Internet", 275.00, "Ahorro Mensual"],
            ["LIBRE / DISPONIBLE PARA TI", 850.00, "Disponible"],
        ],
        columns=[
            "CONCEPTO",
            "MONTO SEMANAL",
    ...
    with r1:
        st.markdown(
            f"""
            <div class="plan-card">
                <div class="plan-title">GASTOS FIJOS</div>
                <div class="plan-value">${GASTO_CASA + GASOLINA + CONSULTA + GASTOS_HIJA:,.2f}</div>
                <div class="plan-sub">
    ...
    with r2:
        st.markdown(
            f"""
            <div class="plan-card">
                <div class="plan-title">AHORRO SEMANAL</div>
                <div class="plan-value">${TOTAL_AHORRO_MENSUAL_SEM:,.2f}</div>
                <div class="plan-sub">
    ...
    with r3:
        st.markdown(
            f"""
            <div class="plan-card">
                <div class="plan-title">LIBRE</div>
                <div class="plan-value">${DISPONIBLE_SEMANAL:,.2f}</div>
                <div class="plan-sub">
    ...
    with col_info:
        ...
        if bloqueado_por_otro_efectivo:
            ...
            st.warning(
                "GITHUB_TOKEN no está configurado. No se puede guardar el corte."
            )
    ...
    if registrar_corte:
        ...
        if not TOKEN:
            ...
            st.error(
                "No existe GITHUB_TOKEN en los Secrets de Streamlit."
            )

        elif not puede_editar_efectivo:

            st.error(
                "No se puede registrar el corte mientras la edición esté bloqueada."
            )
            ...
            try:
                ...
                try:
                    ...
                    contenido_plan = archivo_plan.decoded_content.decode(
                        "utf-8"
                    )

                    df_plan_actual = pd.read_csv(
                        io.StringIO(contenido_plan)
                    )
                ...
                if not df_plan_actual.empty and "Fecha_Corte" in df_plan_actual.columns:

                    fechas_existentes = (
                        df_plan_actual["Fecha_Corte"]
                        .astype(str)
                        .tolist()
                    )
                ...
                numero_semana = ahora_gdl.isocalendar().week

                semana = f"{ahora_gdl.year}-W{numero_semana:02d}"
                ...
                for columna in [
                    ...
                    if columna in df_plan_actual.columns:
                        ...
                        df_plan_actual[columna] = pd.to_numeric(
                            df_plan_actual[columna],
                            errors="coerce"
                        )
                ...
                st.session_state["df_plan_semanal"] = df_plan_actual

                st.success(
                    f"Corte {semana} registrado correctamente en GitHub."
                )
                ...
                st.error(
                    f"No fue posible guardar el corte en GitHub: {e}"
                )
    ...
    if df_hist_plan.empty:
        ...
        df_hist_plan_visual = df_hist_plan[
            columnas_mostrar
        ].copy()
        ...
        for columna in [
            ...
            if columna in df_hist_plan_visual.columns:
                ...
                df_hist_plan_visual[columna] = pd.to_numeric(
                    df_hist_plan_visual[columna],
                    errors="coerce"
                )
