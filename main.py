...
def navigate(main, sub=None):
    ...
    content_render.refresh()
    header_render.refresh() # Refrescamos header para actualizar la línea blanca del menú

# --- COMPONENTES ---

@ui.refreshable
def header_render():
    # El Header DEBE ser hijo directo de la página, pero su CONTENIDO puede ser refrescable
...
@ui.refreshable
def content_render():
    # Este es el contenedor que SÍ puede estar dentro de otros y refrescarse
    with ui.column().classes('w-full items-center px-8 mt-4'):
        # Subnav
...
async def main_page():
    ...
    # Aquí construimos la estructura de la página CUMPLIENDO las reglas de NiceGUI
    header_render()
