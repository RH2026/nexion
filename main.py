...
div[data-testid="stTabs"] [role="tablist"] {{
    ...
    align-items: center !important;
    gap: 36px !important;          /* <-- Espacio entre pestañas */
    margin: 0 !important;
    padding: 0 !important;
}}
...
# ── 3. ¿YA SE LOGUEÓ PERO ES RIGOBERTO Y NO HA ELEGIDO MÓDULO? (PUERTA ÉLITE LOGO CENTRADO) ──
    ...
    with m_col3:
        ...
        if st.button("VISUALIZAR", key="btn_rep", use_container_width=True):
            st.session_state.ejecutivo_modulo = "CORE"; st.session_state.menu_main = "REPORTES"; st.session_state.menu_sub = "ANALISIS MENSUAL"; st.rerun()
    ...
    with m_col4:
        ...
        if st.button("CONFIGURAR", key="btn_hub", use_container_width=True):
            st.session_state.ejecutivo_modulo = "CORE"; st.session_state.menu_main = "CENTRO DE DATOS"; st.session_state.menu_sub = "ASIGNAR FLETERA"; st.rerun()
            ...
            if tipo == "INVENTARIO":
                ...
                if total == 1:
                    ...
                    for index, d in resultados.iterrows():
                        ...
                        st.markdown(f"<div class='card-nexion' style='background:rgba(30,39,46,0.7);border:1px solid rgba(255,255,255,0.05);border-left:4px solid {azul_premium};border-radius:12px;padding:18px 25px;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;'><div style='flex:1;'><span style='color:rgba(255,255,255,0.4);font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;'>PEDIDO / FACTURA</span><br><b style='font-size:18px;color:{azul_premium};letter-spacing:0.5px;'># {d['NÚMERO DE PEDIDO']}</b><br><span style='font-size:10px;color:rgba(255,255,255,0.5);font-weight:600;'>Envío: {d['FECHA DE ENVÍO']}</span></div><div style='flex:2.5;padding-left:25px;border-left:1px solid rgba(255,255,255,0.08);'><span style='color:rgba(255,255,255,0.4);font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;'>CLIENTE / DESTINO</span><br><b style='font-size:13px;color:white;text-transform:uppercase;'>{d['NOMBRE DEL CLIENTE']}</b><br><i style='font-size:11px;color:rgba(255,255,255,0.5);font-style:normal;font-weight:600;'>{d['DESTINO']}</i></div><div style='flex:1.8;padding-left:25px;border-left:1px solid rgba(255,255,255,0.08);'><span style='color:rgba(255,255,255,0.4);font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;'>TRANSPORTE Y GUÍA</span><br><b style='font-size:13px;color:white;text-transform:uppercase;'>{d.get('TRANSPORTE', 'LOGÍSTICA')}</b><br><span style='font-size:12px;color:{azul_premium};font-weight:700;font-family:monospace;'>{d['NÚMERO DE GUÍA']}</span></div><div style='flex:1.2;text-align:right;'><span style='color:rgba(255,255,255,0.4);font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;'>ESTATUS ENTREGA</span><br><b style='font-size:14px;color:{azul_premium};'>{d['FECHA DE ENTREGA REAL']}</b><br><span style='font-size:10px;color:white;font-weight:800;text-transform:uppercase;opacity:0.8;'>{status_text}</span></div></div>", unsafe_allow_html=True)    
    ...
    with main_container:
            ...
            if st.session_state.menu_sub == "ALERTAS":
                ...
                with st.expander("➕ Registrar o Editar Actividad / Incidencia", expanded=False):
                    ...
                    with c3:
                        ...
                        idx_prio = prioridades.index(tarea_existente['IMPORTANCIA']) if tarea_existente is not None else 0
                    ...
                    with st.form("form_nexion_pro_edition", clear_on_submit=True):
                        ...
                        with f3_c1:
                            ...
                            t_grupo = st.text_input("Categoría / Grupo", value=val_grupo)
                        with f3_c2:
                            val_ini = tarea_existente['FECHA'] if tarea_existente is not None else obtener_fecha_mexico()
                            t_ini = st.date_input("Fecha Inicio", value=val_ini)
                        with f3_c3:
        ...
        # 3. REPORTES
            ...
            if st.session_state.menu_sub == "COSTOS CEDIS":
                ...
                [data-testid="stMetric"]{
                    background:#1A252F; padding:25px; border-radius:12px;
                ...
                try:
                    ...
                    if col_concepto and col_cajas:
                        ...
                        total_flete, total_fact, total_cajas = df_final["COSTO DE LA GUIA"].sum(), df_final["FACTURACION"].sum(), df_final[col_cajas].sum()
        ...
        # ── 4. MÓDULO DE FORMATOS (BLOQUE MAESTRO CONSOLIDADO) ────────────────────
            ...
            # --- SUBSECCIÓN C: PROFORMA ---
                ...
                if not df_facturacion.empty:
                    ...
                    with c_col2:
                        if modo_busqueda == "Seleccionar de la lista":
                            num_factura = st.selectbox(
                                ...
                                key="num_factura_lista"
                            )
                            if not df_facturacion.empty and str(num_factura) in df_facturacion["Factura"].values:
                            ...
                            num_factura = st.text_input("✍️ Ingresa Folio Manual", key="num_factura_manual")
                            if (
                                num_factura
                    ...
                    def_cli = str(registro.get("Nombre_Cliente", "")) if not registro.empty and pd.notna(registro.get("Nombre_Cliente", "")) else ""
                    def_fiscal = str(registro.get("FISCAL", "")) if not registro.empty and pd.notna(registro.get("FISCAL", "")) else ""
                    ...
                    with col1:
                        ...
                        rem_cliente = st.text_input("Cliente Remitente", value="JABONES Y PRODUCTOS ESPECIALIZADOS SA DE CV", key="rem_cli")
                        rem_rfc = st.text_input("RFC Remitente", value="JPE830408B35", key="rem_rfc")
                        rem_calle = st.text_input("Calle Remitente", value="Privada del Gallo No. 1525", key="rem_calle")
                      
                        r_col1, r_col2 = st.columns(2)
                        with r_col1:
                            rem_colonia = st.text_input("Colonia Remitente", value="Col La Aurora", key="rem_col")
                        with r_col2:
                            rem_cp = st.text_input("CP Remitente", value="44430", key="rem_cp_val")
                          
                        r_col3, r_col4 = st.columns(2)
                        with r_col3:
                            rem_mun = st.text_input("Municipio Remitente", value="Guadalajara", key="rem_mun")
                        with r_col4:
                            rem_estado = st.text_input("Estado Remitente", value="Jalisco", key="rem_est")
                          
                        r_col5, r_col6 = st.columns(2)
                        with r_col5:
                            rem_contacto = st.text_input("Contacto Remitente", value="Rigoberto Hernandez", key="rem_cont")
                        with r_col6:
                            rem_tel = st.text_input("Teléfono Remitente", value="33 19 75 31 22", key="rem_telf")
                
                    with col2:
                        titulo_seccion("DESTINATARIO / ENTREGA", color_fondo="#4B6B94")
                        dest_cliente = st.text_input("Cliente Destino (Comercial)", value=def_extran, key="dest_cli")
                        dest_rfc = st.text_input("RFC Destino", value=def_rfc, key="dest_rfc")
                        dest_calle = st.text_input("Calle Destino", value=def_dom, key="dest_calle")
                      
                        d_col1, d_col2 = st.columns(2)
                        with d_col1:
                            dest_colonia = st.text_input("Colonia Destino", value=def_col, key="dest_col")
                        with d_col2:
                            dest_cp = st.text_input("CP Destino", value=def_cp, key="dest_cp_val")
                      
                        d_col3, d_col4 = st.columns(2)
                        with d_col3:
                            dest_cui = st.text_input("Ciudad Destino", value=def_cui, key="dest_cui")
                        with d_col4:
                            dest_estado = st.text_input("Estado Destino", value=def_est, key="dest_est")
                          
                        dest_tel = st.text_input("Teléfono Destino", value=tel_val, key="dest_tel")
                    ...
                    fac_cliente = st.text_input("Cliente de Facturación", value=val_cli_fac, key="fac_cli")
                    fac_rfc = st.text_input("RFC Facturación", value=val_rfc_fac, key="fac_rfc")
                    ...
                    def generar_pdf_reportlab():
                        ...
                        title_style = ParagraphStyle("TitleStyle", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=16, leading=18)
                        subtitle_style = ParagraphStyle("SubTitleStyle", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=12, leading=14, alignment=1)
                        th_style = ParagraphStyle("THStyle", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=8.5, leading=10, textColor=colors.white, alignment=1)
                        cell_bold = ParagraphStyle("CellBold", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=7.5, leading=9)
                        cell_normal = ParagraphStyle("CellNormal", parent=styles["Normal"], fontName="Helvetica", fontSize=7.5, leading=9)
                        ...
                        if logo_io:
                            ...
                            logo_element = Paragraph(
                                ...
                                ParagraphStyle("FallbackLogo", parent=styles["Normal"], fontName="Helvetica-BoldOblique", fontSize=16, textColor=colors.HexColor("#003366"), alignment=2)
                            )
                
                        header_data = [[Paragraph("PAQMEX S.A. DE C.V.", title_style), logo_element]]
                        t_header = Table(header_data, colWidths=[280, 274])
                        t_header.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("ALIGN", (1, 0), (1, 0), "RIGHT"), ("BOTTOMPADDING", (0, 0), (-1, -1), 0), ("TOPPADDING", (0, 0), (-1, -1), 0)]))
                        ...
                        meta_data = [
                            ["", Paragraph("<b>FECHA:</b>", ParagraphStyle("R", alignment=2, fontSize=9)), Paragraph(f"<b>{fecha_actual}</b>", ParagraphStyle("C", alignment=1, fontSize=9))],
                            ["", Paragraph("<b>FACTURA:</b>", ParagraphStyle("R", alignment=2, fontSize=9)), Paragraph(f"<b>{num_factura}</b>", ParagraphStyle("C", alignment=1, fontSize=9))],
                        ]
                        t_meta = Table(meta_data, colWidths=[350, 100, 104])
                        t_meta.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("LINEBELOW", (2, 0), (2, 0), 1, colors.black), ("LINEBELOW", (2, 1), (2, 1), 1, colors.black), ("BOTTOMPADDING", (0, 0), (-1, -1), 2)]))
                        story.append(t_meta)
                        story.append(Spacer(1, 6))
                
                        rem_data = [
                            [Paragraph("REMITENTE", th_style), ""],
                            [Paragraph("CLIENTE:", cell_bold), Paragraph(remitente["cliente"], cell_bold)],
                            ...
                            [Paragraph("COLONIA:", cell_bold), Paragraph(remitente["colonia"], cell_normal)],
                            [Paragraph("MUNICIPIO:", cell_bold), Paragraph(f"{remitente['municipio']} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>ESTADO:</b> {remitente['estado']}", cell_normal)],
                            [Paragraph("CONTACTO:<br/>TELEFONO:", cell_bold), Paragraph(f"{remitente['contacto']}<br/>{remitente['telefono']}", cell_normal)],
                        ]
                        t_rem = Table(rem_data, colWidths=[70, 202])
                        t_rem.setStyle(TableStyle([("SPAN", (0, 0), (1, 0)), ("BACKGROUND", (0, 0), (1, 0), colors.HexColor("#6c8ebf")), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("TOPPADDING", (0, 0), (-1, -1), 3), ("BOTTOMPADDING", (0, 0), (-1, -1), 3)]))
                
                        dest_data = [
                            [Paragraph("DESTINATARIO", th_style), ""],
                            [Paragraph("CLIENTE:", cell_bold), Paragraph(destinatario["cliente"], cell_bold)],
                            [Paragraph("RFC:", cell_bold), Paragraph(destinatario["rfc"], cell_normal)],
                            [Paragraph("CALLE:", cell_bold), Paragraph(destinatario["calle"], cell_normal)],
                            [Paragraph("COLONIA:", cell_bold), Paragraph(destinatario["colonia"], cell_normal)],
                            [Paragraph("MUNICIPIO:", cell_bold), Paragraph(f"{destinatario['municipio']} &nbsp;&nbsp; {destinatario['estado']}", cell_normal)],
                            [Paragraph("TELEFONO:", cell_bold), Paragraph(destinatario["telefono"], cell_normal)],
                        ]
                        t_dest = Table(dest_data, colWidths=[70, 202])
                        t_dest.setStyle(TableStyle([("SPAN", (0, 0), (1, 0)), ("BACKGROUND", (0, 0), (1, 0), colors.HexColor("#6c8ebf")), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("TOPPADDING", (0, 0), (-1, -1), 3), ("BOTTOMPADDING", (0, 0), (-1, -1), 3)]))
                
                        t_top_blocks = Table([[t_rem, t_dest]], colWidths=[274, 274])
                        t_top_blocks.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP"), ("LEFTPADDING", (0, 0), (-1, -1), 0), ("RIGHTPADDING", (0, 0), (-1, -1), 0)]))
                        story.append(t_top_blocks)
                        story.append(Spacer(1, 6))
                
                        fac_data = [
                            [Paragraph("FACTURACION", th_style), ""],
                            [Paragraph("CLIENTE:", cell_bold), Paragraph(facturacion["cliente"], cell_bold)],
                            [Paragraph("DATOS / RFC:", cell_bold), Paragraph(f"{facturacion.get('calle', '')}<br/><b>RFC: {facturacion['rfc']}</b>", cell_normal)],
                            [Paragraph("EMAIL:", cell_bold), Paragraph(facturacion.get("email", "sbomailer@jypesa.com"), cell_normal)],
                        ]
                        t_fac = Table(fac_data, colWidths=[70, 202])
                        t_fac.setStyle(TableStyle([("SPAN", (0, 0), (1, 0)), ("BACKGROUND", (0, 0), (1, 0), colors.HexColor("#6c8ebf")), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("TOPPADDING", (0, 0), (-1, -1), 4), ("BOTTOMPADDING", (0, 0), (-1, -1), 4)]))
                        ...
                        t_serv.setStyle(TableStyle([("SPAN", (0, 0), (3, 0)), ("BACKGROUND", (0, 0), (3, 0), colors.HexColor("#6c8ebf")), ("SPAN", (2, 2), (3, 2)), ("SPAN", (2, 3), (3, 3)), ("SPAN", (2, 4), (3, 4)), ("SPAN", (0, 6), (3, 6)), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("ALIGN", (1, 1), (1, 3), "CENTER"), ("ALIGN", (1, 5), (1, 5), "CENTER"), ("TOPPADDING", (0, 0), (-1, -1), 3.2), ("BOTTOMPADDING", (0, 0), (-1, -1), 3.2)]))
                
                        t_mid_blocks = Table([[t_fac, t_serv]], colWidths=[274, 274])
                        t_mid_blocks.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP"), ("LEFTPADDING", (0, 0), (-1, -1), 0), ("RIGHTPADDING", (0, 0), (-1, -1), 0)]))
                        story.append(t_mid_blocks)
                        story.append(Spacer(1, 6))
                
                        cont_data = [[Paragraph("CONTENIDO", th_style), "", "", "", "", "", ""]]
                        cont_headers = ["#", "CANTIDAD", "EMPAQUE", "CONTENIDO", "DIMENSIONES (ALTO/LARGO/ANCHO)", "KG REAL", "KG VOLUMEN"]
                        ...
                        t_cont.setStyle(TableStyle([("SPAN", (0, 0), (6, 0)), ("BACKGROUND", (0, 0), (6, 0), colors.HexColor("#6c8ebf")), ("BACKGROUND", (0, 1), (6, 1), colors.HexColor("#6c8ebf")), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("ALIGN", (0, 0), (-1, -1), "CENTER"), ("TOPPADDING", (0, 0), (-1, -1), 3), ("BOTTOMPADDING", (0, 0), (-1, -1), 3)]))
                        story.append(t_cont)
                        story.append(Spacer(1, 6))
                
                        cp_data = [[Paragraph("CARTA PORTE", th_style), "", ""]]
                        cp_headers = ["#", "CODIGO PRODUCTO CARTA PORTE SAT", "CODIGO UNIDAD PESO CARTA PORTE SAT"]
                        cp_data.append([Paragraph(h, th_style) for h in cp_headers])
                        for i in range(1, 5):
                            cp_data.append([str(i), "", ""])
                
                        t_cp = Table(cp_data, colWidths=[25, 261, 262])
                        t_cp.setStyle(TableStyle([("SPAN", (0, 0), (2, 0)), ("BACKGROUND", (0, 0), (2, 0), colors.HexColor("#6c8ebf")), ("BACKGROUND", (0, 1), (2, 1), colors.HexColor("#6c8ebf")), ("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("ALIGN", (0, 0), (-1, -1), "CENTER"), ("TOPPADDING", (0, 0), (-1, -1), 3), ("BOTTOMPADDING", (0, 0), (-1, -1), 3)]))
                        story.append(t_cp)
                        story.append(Spacer(1, 30))
                
                        sig_style = ParagraphStyle("Sig", parent=styles["Normal"], fontName="Helvetica-Bold", fontSize=7.5, leading=9, alignment=1)
                        sig_data = [
                            [
                                Paragraph("________________________________________<br/>FIRMA Y NOMBRE DEL CLIENTE :", sig_style),
                                Paragraph("________________________________________<br/>FIRMA Y NOMBRE DE QUIEN RECIBE :", sig_style),
                                Paragraph("________________________________________<br/>NUMERO DE UNIDAD", sig_style),
                            ]
                        ]
                        t_sig = Table(sig_data, colWidths=[183, 183, 182])
                        t_sig.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP"), ("ALIGN", (0, 0), (-1, -1), "CENTER")]))
                        story.append(t_sig)
                    ...
                    st.success(f"¡Orden de embarque para la factura {num_factura} lista para descargar!")
                    st.download_button(
                        ...
                        mime="application/pdf",
                        use_container_width=True,
                        key="btn_descargar_pdf_preguia"
                    )
                else:
                    st.warning("No se encontraron datos en el CSV de GitHub o el archivo está vacío.")
            ...
            """, unsafe_allow_html=True)
               ...
               # --- CONFIGURACIÓN DE RECOLECCIONES ONE PAQUETERÍA (ACTUALIZADO Y BLINDADO CON COMENTARIOS) ---
                @st.cache_data(ttl=60)
                def cargar_csv_github():
                    try:
                        repo = "RH2026/nexion"
                ...
                if not df_facturacion.empty:
                    ...
                    # --- NUEVA ÁREA DE COMENTARIOS ---
                    st.markdown("---")
                    titulo_seccion("💬 OBSERVACIONES / COMENTARIOS ADICIONALES", color_fondo="#37474f")
                    comentarios_usuario = st.text_area(
                        "Instrucciones o notas especiales para la recolección",
                        value="LLAMAR AL REMITENTE UNA HORA ANTES DE LA RECOLECCIÓN, SI NO QUIEREN ENTREGAR LLAMAR AL TELÉFONO Cel. 33 19 75 31 22 Rigoberto Hernandez",
                        key="one_comentarios"
                    )
                    ...
                    # --- FUNCIÓN PDF REPORTLAB (ESTILO ONE PAQUETERÍA CORREGIDO) ---
                    def generar_pdf_one_paqueteria():
                        ...
                        # 1. ENCABEZADO SUPERIOR (Ajustado exactamente a 582 pts de ancho total)
                        logo_io = obtener_logo_one()
                        logo_elem = Image(logo_io, width=85, height=22) if logo_io else Paragraph("<b>ONE Paquetería</b>", cell_center)
                        ...
                        t_rem = Table(rem_data, colWidths=[85, 206])
                        t_rem.setStyle(TableStyle([
                            ("SPAN", (0,0), (1,0)),
                        ...
                        t_dest = Table(dest_data, colWidths=[85, 206])
                        t_dest.setStyle(TableStyle([
                            ("SPAN", (0,0), (1,0)),
                        ...
                        t_top = Table([[t_rem, t_dest]], colWidths=[291, 291])
                        story.append(t_top)
                        story.append(Spacer(1, 2))
                        ...
                        # 7. BLOQUE FINAL (Con los comentarios dinámicos del usuario)
                        t_final_block = Table([
                            [Paragraph("<b>DATOS DE QUIEN SOLICITA EL SERVICIO</b>", th_blue), Paragraph("<b>OBSERVACIONES / COMENTARIOS</b>", th_red)],
                            [
                                Table([
                                    [Paragraph("<b>NOMBRE:</b>", cell_bold), Paragraph("RIGOBERTO HERNANDEZ", cell_center)],
                                    [Paragraph("<b>EMPRESA:</b>", cell_bold), Paragraph("JYPESA", cell_center)],
                                ...
                                Paragraph(f"<b>{comentarios_usuario}</b>", cell_normal)
                            ]
                        ], colWidths=[290, 292])
                        t_final_block.setStyle(TableStyle([
                    ...
                    if st.button("🚀 Generar Manifiesto de Embarque (ONE Paquetería)", use_container_width=True, key="one_btn_generar_pdf"):
                        ...
                        st.success("¡Manifiesto de ONE Paquetería generado con éxito con tus observaciones!")
                        st.download_button(
        ...
        # 5. HUB LOG
            ...
            if st.session_state.menu_sub == "ASIGNAR FLETERA":               
                    ...
                    try:
                        ...
                        if r.status_code == 200:
                            ...
                            if not df_kpi.empty:
                                ...
                                # --- MAGIA CORREGIDA PARA SELECCIONAR EL MES ACTUAL ---
                                from datetime import datetime
                                mes_actual_str = datetime.now().strftime('%Y-%m') # Esto nos da "2026-06" exactamente
                                
                                # Verificamos buscando el string exacto en tu lista
                                if mes_actual_str in opciones_mes_muestras:
                                    indice_defecto = opciones_mes_muestras.index(mes_actual_str) + 1
                                else:
                                    indice_defecto = 0 
