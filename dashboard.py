            ...
            if df_raw is not None:
                ...
                with tab_rastreo:
                    ...
                    st.markdown(f"""
                        ...
                        /* Estilo para los filtros manuales */
                        .filter-label {{ color: {vars_css['sub']}; font-size: 10px; font-weight: 700; margin-bottom: 5px; }}
                        </style>
                    """, unsafe_allow_html=True)
                    ...
                    with col_search:
                        busqueda_manual = st.text_input("", key="busqueda_master", placeholder="🔍 Búsqueda rápida (Factura, Guía o Cliente)...").strip()
                    
                    # --- 2. LÓGICA DE FILTRADO DINÁMICO ---
                    df_filtrado = df_raw.copy()
                    
                    # Filtro por caja principal
                    if busqueda_manual:
                        mask_master = (df_filtrado["NÚMERO DE PEDIDO"].astype(str).str.contains(busqueda_manual, case=False)) | \
                                      ...
                                      (df_filtrado["NOMBRE DEL CLIENTE"].astype(str).str.contains(busqueda_manual, case=False))
                        df_filtrado = df_filtrado[mask_master]
                    
                    # --- 3. RENDERIZADO DEL TIMELINE (ARRIBA) ---
                    # Solo se muestra si hay una búsqueda en la caja principal y hay resultados
                    ...
                    # --- 4. FILTROS MANUALES POR COLUMNA (SOLO PARA LA TABLA) ---
                    st.markdown(f"<div style='color:{vars_css['sub']}; font-size:10px; font-weight:700; margin-bottom:10px;'>FILTROS AVANZADOS POR COLUMNA</div>", unsafe_allow_html=True)
                    col_f1, col_f2, col_f3, col_f4 = st.columns(4)
                    
                    with col_f1:
                        f_cliente = st.text_input("Cliente", key="f_cli", placeholder="Filtrar cliente...")
                    with col_f2:
                        f_destino = st.text_input("Destino", key="f_des", placeholder="Filtrar ciudad...")
                    with col_f3:
                        f_fletera = st.text_input("Fletera", key="f_fle", placeholder="Filtrar fletera...")
                    with col_f4:
                        f_guia = st.text_input("Nº Guía", key="f_gui", placeholder="Filtrar guía...")
                    
                    # Aplicar filtros manuales a la tabla
                    if f_cliente: df_filtrado = df_filtrado[df_filtrado["NOMBRE DEL CLIENTE"].astype(str).str.contains(f_cliente, case=False)]
                    if f_destino: df_filtrado = df_filtrado[df_filtrado["DESTINO"].astype(str).str.contains(f_destino, case=False)]
                    if f_fletera: df_filtrado = df_filtrado[df_filtrado["FLETERA"].astype(str).str.contains(f_fletera, case=False)]
                    if f_guia: df_filtrado = df_filtrado[df_filtrado["NÚMERO DE GUÍA"].astype(str).str.contains(f_guia, case=False)]
