            ...
            if st.session_state.menu_sub == "ALERTAS":
                ...
                with st.expander(":material/view_list: Monitor de Pendientes NEXION", expanded=True):
                    ...
                    for index, row in df_master.iterrows():
                        ...
                        # HTML ELITE COMPRIMIDO (UNA SOLA LÍNEA PARA MÁXIMA ESTABILIDAD)
                        html_card = """<div class="task-card" style="border-left-color: {color}; padding: 20px; margin-bottom: 20px; border-radius: 12px;"><div style="display: flex; justify-content: space-between; align-items: center; width: 100%;"><div style="flex: 1; border-right: 1px solid rgba(255,255,255,0.1); padding-right: 15px;"><div style="font-size: 10px; color: {color}; font-weight: 800; letter-spacing: 1px;">FOLIO / PEDIDO</div><div style="font-size: 18px; font-weight: 800; color: white;">{folio}</div><div style="font-size: 9px; color: rgba(255,255,255,0.4);">REF: {grupo}</div></div><div style="flex: 3; padding: 0 20px; border-right: 1px solid rgba(255,255,255,0.1);"><div style="font-size: 10px; color: rgba(255,255,255,0.4); font-weight: 800;">CLIENTE / GUÍA / DESTINO</div><div style="font-size: 13px; font-weight: 700; color: #e2e8f0; line-height: 1.2;">{desc}</div><div style="font-size: 10px; color: #38bdf8; margin-top: 5px; font-weight: 600;">👤 {user} | 🗓️ {inicio} ➔ {fin}</div></div><div style="flex: 1.8; padding-left: 20px; text-align: right;"><div style="font-size: 9px; color: {color}; font-weight: 800; letter-spacing: 1px;">ESTATUS ACTUAL</div><div style="font-size: 14px; color: white; font-weight: 700; font-style: italic; margin-bottom: 8px; line-height: 1.1;">"{accion}"</div><div style="display: flex; align-items: center; justify-content: flex-end; gap: 8px;"><div style="background: rgba(255,255,255,0.1); border-radius: 10px; height: 5px; width: 100px;"><div style="height: 5px; border-radius: 10px; width: {prog}%; background: {color}; box-shadow: 0 0 10px {shd};"></div></div><span style="font-size: 11px; font-weight: 800; color: white;">{prog}%</span></div></div></div></div>""".format(color=color_p, folio=folio_display, grupo=row['GRUPO'], desc=desc_limpia, user=row['USUARIO'], inicio=row['FECHA'], fin=row['FECHA_FIN'], accion=row['ULTIMO ACCION'], prog=progreso, shd=sombra)
