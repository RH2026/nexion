            ...
            if st.session_state.menu_sub == "ALERTAS":
                ...
                if df_master.empty:
                    ...
                    for _, row in df_master.iterrows():
                        ...
                        st.markdown(f"""<div class="card-hover" style="border-left: 5px solid {color_p}; padding: 15px; margin-bottom: 10px; background: #262e33; border-radius: 5px; border: 1px solid #3d474d;"><div style="display: grid; grid-template-columns: 1fr 2fr 1.5fr 1fr; gap: 15px; align-items: center;"><div><div style="font-size: 0.7em; color: #888;">FOLIO / ESTATUS</div><div style="color: {color_p}; font-weight: bold; font-size: 1.1em;">{row.get('FOLIO', 'INC-???')}</div><span style="background: {color_e}33; color: {color_e}; border: 1px solid {color_e}; padding: 2px 6px; border-radius: 3px; font-weight: bold; font-size: 0.7em;">{f_est}</span></div><div><div style="font-size: 0.7em; color: #888;">CLIENTE / PEDIDO</div><div style="color: #fff; font-weight: bold;">{row.get('CLIENTE_DESTINO', 'N/A')}</div><div style="font-size: 0.8em; color: #bbb;">📦 {row.get('PEDIDO_GUIA', 'N/A')}</div></div><div><div style="font-size: 0.7em; color: #888;">DETALLE / ACCIONES</div><div style="font-size: 0.85em; color: #eee;">{row.get('DETALLE_INCIDENCIA', 'Sin detalle...')}</div><div style="font-size: 0.85em; color: #38bdf8;"><i>{row.get('ACCIONES', '')}</i></div></div><div style="text-align: right;"><div style="font-size: 0.7em; color: #888;">RESPONSABLE / REGISTRÓ</div><div style="color: #fff; font-size: 0.9em;">👤 {row.get('RESPONSABLE', 'N/A')}</div><div style="font-size: 0.7em; color: #38bdf8;">📝 {row.get('USUARIO', 'N/A')}</div></div></div></div>""", unsafe_allow_html=True)
